...
int main(int argc, char *argv[])
{
	...
	// Add translator
	// ** creation of translator must be after app and before window
	 
	QTranslator* main_tr = new QTranslator();
	if (main_tr->load("meshlab_zh.qm")) {
		qDebug() << "load meshlab_zh.qm sucessfull";
		app.installTranslator(main_tr);
	}

	QTranslator* editAlign_tr = new QTranslator();
	if (editAlign_tr->load("../meshlabplugins/edit_align/edit_align_zh.qm")) {
		qDebug() << "load edit_align_zh.qm sucessfull";
		app.installTranslator(editAlign_tr);
	}

	QTranslator* deco_tr = new QTranslator();
	if (deco_tr->load("../meshlabplugins/decorate_base/decorate_base_zh.qm")) {
		qDebug() << "load decorate_base_zh.qm sucessfull";
		app.installTranslator(deco_tr);
	}

	QTranslator* editMain_tr = new QTranslator();
	if (editMain_tr->load("../meshlabplugins/edit_mainplugin/edit_main_zh.qm")) {
		qDebug() << "load edit_main_zh.qm sucessfull";
		app.installTranslator(editMain_tr);
	}
