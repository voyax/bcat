...
//static std::vector<QColor> contourColors; //二维图形的颜色，设置8种
static QColor getContourColors(int meshNumber, int i) {
	if (meshNumber == 1) {
		return QColor(29, 80, 158); // Dark Blue
	}
	if (meshNumber == 2) {
		if (i == 0) {
...
// 用来显示GrowthChart 的界面
class GrowthChartWidget :public QFrame {
public:
	GrowthChartWidget(QWidget* parent = 0);
	MainWindow* mw();
	QString qtReportName = QString();
	bool qtReportIsBoy = true;
	QString qtReportUserId = QString();
	QString qtReportId = QString();
	QDate qtReportBirthDay = QDate();
	QDate qtReportScanDate = QDate();
	int qtGestationWeeks = 0;
	int qtGestationDays = 0;
	double qtAdjustedAge = 0;
	QString qtReportPhoneNumber = QString();
	double headCir;
...
private slots:
	...
	void slotAddNewDockWidget(bool show=true);
	void slotIniteSliceValue();
	void slotUpdateSliceValue(int level);
	void slotExportReport();
	void slotExportReportLight();
	void slotExportReportHelper(bool simplified=true);
	void slotExportCsv();
	void slotSnapshotBackFirst();
	void slotSnapshotFrontFirst();
	void slotSnapshotTopFirst();
	void slotSnapshotLeftFirst();
	void slotSnapshotBackPrev();
	void slotSnapshotFrontPrev();
	void slotSnapshotTopPrev();
	void slotSnapshotLeftPrev();
	void slotSnapshotBackCurr();
	void slotSnapshotFrontCurr();
	void slotSnapshotTopCurr();
	void slotSnapshotLeftCurr();
	void slotSnapshotViewHelper(QString view, QString model);
	void slotUpdateGrowthChart();
...
public slots:
	...
	// slice every model
	void slotSliceEveryModel();
	void slotCompareModel();
	void slotMeshSetChanged();
...
private slots:
	...
	bool importMeshHelper(QString filename = QString());
	void endEdit();
...
private:
	...
	*/
	...
	// 切片后的测量信息
	QDockWidget* sliceDockWindow;
	Ui::SliceDialog sliceUI;
	SliceWidget* sliceWidget;
	GrowthChartWidget* growthChartWidget;
	std::vector<QToolButton*> stageButtons;
	std::vector<QToolButton*> stageButtons_shadow;
