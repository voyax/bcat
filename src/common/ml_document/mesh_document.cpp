...
void MeshDocument::sortMeshListByName() {
	meshList.sort([=](MeshModel a, MeshModel b) {
		QString stepStrA = a.fullName();
		stepStrA = stepStrA.mid(stepStrA.lastIndexOf('_') + 1, stepStrA.lastIndexOf('.') - 1 - stepStrA.lastIndexOf('_'));
		bool convertSuccessA = true;
		int stepA = stepStrA.toInt(&convertSuccessA, 10);

		QString stepStrB = b.fullName();
		stepStrB = stepStrB.mid(stepStrB.lastIndexOf('_') + 1, stepStrB.lastIndexOf('.') - 1 - stepStrB.lastIndexOf('_'));
		bool convertSuccessB = true;
		int stepB = stepStrB.toInt(&convertSuccessB, 10);

		if (convertSuccessA && convertSuccessB) {
			return stepA < stepB;
		}
		else {
			return a.id() < b.id();
