...
public:
	...
	*/
	...
	//每个MeshModel的绘制方式、测量数据等
	struct Measurement {
		std::vector<float> c;
		std::vector<float> cr_L;
		std::vector<float> cr_W;
		std::vector<float> cr;
		std::vector<float> rsi;
		std::vector<float> cvai_d1;
		std::vector<float> cvai_d2;
		std::vector<float> cvai;
		...
		float asr;
		float psr;
		float overall_symmetry_ratio;
		float ap_volume_ratio;
		float head_height;

		std::vector<std::vector<vcg::Point3f>> cvai_points; // levels of vectors of points

		Measurement() {
			volume_q1 = volume_q2 = volume_q3 = volume_q4 = -1; // 未赋值参数
			asr = psr = overall_symmetry_ratio = ap_volume_ratio = head_height = -1; // 未赋值参数
			c.resize(10, -1);
			cr_L.resize(10, -1);
			cr_W.resize(10, -1);
			cr.resize(10, -1);
